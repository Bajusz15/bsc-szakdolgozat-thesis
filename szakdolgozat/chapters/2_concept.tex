...
A fejlesztést olyan szakemberek vezették, akik korábban a Unix operációs rendszer, a Java HotSpot JVM vagy az UTF-8 karakterkódolás fejlesztésében is kulcsszerepet játszottak.
Például Ken Thompson, Rob Pike és Robert Griesemer.
A cél az volt, hogy régebbi általános célú nyelvek hiányosságait kiküszöböljék, hiszen a megváltozott üzleti és technológiai körülmények között ezek a nyelvek nem bizonyultak elég hatékonynak.
...
időben és földrajzi elhelyezkedésben is megosztva.
Szintaktikailag a C-hez hasonlít de nagyon könnyen tanulható nyelv.
A legnagyobb újítás a konkurrencia mechanizmus és hibakezelés volt.

A Go konkurrencia mechanizmusa remekül képes kihasználni a többmagos processzorok
és konkurens futtatás adta számítási kapacitást.
Megkönnyíti olyan programok írását amelyet a legtöbbet hozzák ki a többmagos és hálózaton összekötött gépekből.
...
A konkurrencia a nyelv alapjának része. A két legfontosab dolog ebből a szempontból a gorutinok (`goroutine`) és csatornák (`channel`).
...
\end{python}
A fenti függvény például egy File típus pointerét és egy error típust ad vissza. Ha nem futott hibába a program, az err változó nil értéket kap.
