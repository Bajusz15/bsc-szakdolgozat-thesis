...
func (s *Storage) GetTelemetriesByDrone(droneID int) ([]models.Telemetry, error) {
	...
	err := s.db.Select(&t, `SELECT id, drone_id, speed, location, altitude, compass_direction, acceleration, battery_level, battery_temperature, motor_temperatures, time_stamp FROM telemetry`)
	if err != nil {
		return nil, err
	}
	return nil, nil
...
func (s *Storage) InsertTelemetry(droneID int, t models.Telemetry) error {
	_, err := s.db.Exec(`INSERT INTO telemetry (drone_id, speed, location, altitude, compass_direction, acceleration, battery_level,
		...
		droneID, t.Speed, t.Location, t.Altitude, t.CompassDirection, t.Acceleration, t.BatteryLevel,
